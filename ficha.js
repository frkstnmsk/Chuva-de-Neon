...
function renderizarAtributos(modificadoresPlanos) {
    ...
    // toda rolagem de perícia. Com o Godmode ativo (godmodeAtivo, ver
    // configurarGodmode), a penalidade é totalmente desligada — pra
    // qualquer ficha, jogador ou Mestre — mesmo que o personagem esteja
    // com o PV baixo.
    const temTolerancia = temPericiaTreinada(fichaAtual.pericias, "Tolerância");
    const estadoSaude = calcularEstadoSaude(d.pvAtual, pvMaximoTotal, temTolerancia, godmodeAtivo);
...
async function resolverAtaque(it, modificadoresPlanosAtacante, participante) {
    ...
    const modPericia = modificadorDePericiaComPenalidade(nomePericia, fichaAtual.dados, fichaAtual.pericias, modificadoresPlanosAtacante, penalidadeTestesAtual());
    ...
    // agindo por um NPC), igual em qualquer outra rolagem.
    if (participanteIdParaGastarAcao) {
        if (consumo.direto) {
            await consumirAcaoCombate(participanteIdParaGastarAcao);
        } else {
