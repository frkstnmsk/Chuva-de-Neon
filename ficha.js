...
import {
    ...
    calcularEspecificidadeGolpe, bonusEsquivaBoxe
...
function renderizarAtributos(modificadoresPlanos) {
    const d = fichaAtual.dados;
    // BUGFIX: isto tinha ficado como podeEditarPericiaAtributo(), que é
    // true durante toda a Criação e o Level Up inteiro (passo 1 a 3, não
    // só o passo de atributo) e durante Treinamento. Como este campo é
    // um <input type="number"> cru, sem NENHUM teto embutido, isso
    // deixava o jogador digitar qualquer valor (99, 9999...) direto no
    // atributo enquanto qualquer uma dessas janelas estivesse aberta —
    // era o "upar atributo infinito no Level Up" relatado. A mudança de
    // atributo de verdade sempre passa por outro caminho (stepper da
    // Etapa 2 da Criação, botão do Passo 1 do Level Up, ou o
    // distribuidor de pontos bônus) — todos eles já têm teto e descontam
    // do pool certo. Este campo cru só devia mesmo ser editável pelo
    // Mestre em godmode, como o comentário abaixo sempre disse.
    const podeEditarAtributoDireto = isMestre && godmodeAtivo;

    ATRIBUTOS_PRIMARIOS.forEach(attr => {
        const input = document.querySelector(`[data-attr-primario="${attr.key}"]`);
        if (!input) return;
        if (document.activeElement !== input) input.value = d[attr.key] ?? 0;
        input.disabled = !podeEditarAtributoDireto;
        input.closest(".attr-card").classList.toggle("locked", !podeEditarAtributoDireto);
    });
...
// de sempre: só rola a perícia, sem alvo.
...
// fogo − Percepção do atacante, ou 10 + Agilidade/Constituição do alvo
// pra corpo a corpo/desarmado), e se acertar, resolve o dano (arma ou
...
async function resolverAtaque(it, modificadoresPlanosAtacante, participante) {
    ...
    try {
        if (participante.tipo === "ficha") {
            ...
            if (ehFogo) {
                ...
                dificuldade = calcularDificuldadeDefesaJogador(fichaAlvo.dados, atributoDefesaChave, modsAlvo);
            }
        } else {
    ...
    if (combateComIniciativaAtivo() && participante.esquivaDisponivel) {
        ...
        if (escolha === "E" || escolha === "B") {
            ...
            if (usou !== false) {
                if (escolha === "E") {
                    const detalheAnulado = `${nomeAtacante} atacou ${nomeAlvo} com ${it.nome} (${nomePericia}). ACERTO! (${resultadoAtaque} vs. dificuldade ${dificuldade}${recuoTexto}${precisaoTexto}) — mas ${nomeAlvo} usou a ação guardada pra ESQUIVAR e ANULOU o golpe.`;
                    await registrarRolagem({ quem: nomeAtacante, modificador: modAtaque, resultado: 0, detalhe: detalheAnulado });
                    toast(detalheAnulado);
                    return;
                }
                // Bloquear
                if (tipoDanoKey === "perfuracao_comum" || tipoDanoKey === "perfuracao_especial") {
                    toast(`${nomeAlvo} tentou Bloquear, mas dano perfurante não é reduzido por bloqueio. Ação guardada consumida mesmo assim.`);
                } else {
    ...
    const detalheDano = resultadoDano.reducao > 0
        ? `${nomeAtacante} atacou ${nomeAlvo} com ${it.nome}. ACERTO! (${resultadoAtaque} vs. dificuldade ${dificuldade}${recuoTexto}${precisaoTexto}) Dano: ${resultadoDano.danoBruto} (${tipoDanoLabel}) - ${resultadoDano.reducao} (redução) = ${resultadoDano.danoFinal} de dano aplicado. PV restante: ${resultadoDano.novoPv}.${efeitoTexto}`
        : `${nomeAtacante} atacou ${nomeAlvo} com ${it.nome}. ACERTO! (${resultadoAtaque} vs. dificuldade ${dificuldade}${recuoTexto}${precisaoTexto}) Dano: ${resultadoDano.danoFinal} (${tipoDanoLabel}) aplicado. PV restante: ${resultadoDano.novoPv}.${efeitoTexto}`;
...
function renderizarInventario(modificadoresPlanos) {
    ...
    if (!itensCategoria.length) {
        ...
        itensCategoria.forEach(([id, it]) => {
            ...
            const classeLabel = it.classeProtecao ? ` · Classe de Proteção ${escapeHtml(it.classeProtecao)}` : "";
            ...
            li.innerHTML = `
                <div class="entity-main">
                    ...
                    <span class="entity-sub">${tagLabel} · ${it.peso || 0} kg${periciaLabel}${classeLabel}${reducaoLabel}</span>
                </div>
                <div class="entity-badges">
...
function renderizarCombate() {
    ...
    if (!armas.length) {
        ...
        armas.forEach(arma => {
            ...
            const classeLabel = arma.classeProtecao ? ` · Classe de Proteção ${escapeHtml(arma.classeProtecao)}` : "";
            const ehFogo = ehArmaDeFogo(arma.periciaUso);
            const fogoLabel = ehFogo
                ? ` · Dif. acerto ${cfg.dificuldadeAcerto ?? "—"} · Alcance ${rotuloAlcanceArmaFogo(cfg.alcance)} · Recuo: ${rotuloPadraoRecuo(cfg.recuo)}${cfg.precisao ? ` · Precisão ${cfg.precisao >= 0 ? "+" : ""}${cfg.precisao}` : ""}`
                : "";
            li.innerHTML = `
                <div class="entity-main">
        ...
        li.querySelectorAll("[data-pericia-golpe]").forEach(btn => {
            btn.addEventListener("click", async (e) => {
                ...
                if (ehGolpeDesarmadoComDano(m.nome)) {
                    ...
                    const nivelPericia = Number(entrada[1].nivel) || 0;
                    ...
                    if (combateTemParticipantes()) {
                        ...
                        await rolarERegistrar(`${m.nome} (${nomePericia}) · dano potencial ${danoCalc.total} (${dadoTexto} + ${danoCalc.bonusEscala})`, calc.total);
                    }
                    return;
                }
...
// Atributos primários — input cru, só pro Mestre em godmode (ver
// comentário em renderizarAtributos: qualquer alteração de atributo de
// jogador tem que passar pelo wizard, pelo Level Up ou pelo distribuidor
// de pontos bônus, que já descontam do pool certo e têm teto).
document.addEventListener("input", (e) => {
    const attrKey = e.target.dataset && e.target.dataset.attrPrimario;
    if (!attrKey || !fichaAtualId) return;
...
function prepararModalPericia(existente) {
    ...
    // BUGFIX: nível só é livre pro Mestre. Ver salvarPericiaDoModal pra
    // detalhes — isto aqui é só a trava visual (a trava de verdade é lá).
    el.modalNivel.disabled = !isMestre;
...
function prepararModalItem(existente, ehBanco) {
    ...
    if (existente) {
        ...
        atualizarCamposPorTag(existente.tag, existente.nivelTag, existente.arma, existente.periciaUso, existente.classeProtecao, existente.reducoesDano);
    } else {
        el.modalNome.value = "";
        el.modalTag.value = "";
        el.modalPeso.value = 0;
        if (!ehBanco) el.modalCategoriaItem.value = categoriaInventarioAtiva || "levando";
        atualizarCamposPorTag("", null, null, null, null, null);
    }
    ...
    const renderSugestoes = () => {
        ...
        encontrados.forEach(it => {
            ...
            div.addEventListener("click", () => {
                ...
                atualizarCamposPorTag(it.tag, it.nivelTag, it.arma, it.periciaUso, it.classeProtecao, it.reducoesDano);
                el.modalItemBancoOpcoes.style.display = "none";
...
function atualizarCamposPorTag(tagKey, nivelTag, armaConfig, periciaUsoAtual, classeProtecaoAtual, reducoesDanoAtuais) {
...
document.getElementById("modal-tag")?.addEventListener("change", (e) => {
    atualizarCamposPorTag(e.target.value, null, null, null, null, null);
});
...
async function salvarPericiaDoModal(id) {
    ...
    // BUGFIX: o nível de uma perícia só pode subir através do stepper da
    // Etapa 3/4 da Criação, do Passo 3 do Level Up, ou do distribuidor de
    // pontos bônus — todos eles mexem direto em fichaAtual.pericias e já
    // descontam de um pool com teto próprio. Este modal genérico (aberto
    // pelo "+ Adicionar Perícia" ou clicando numa perícia da lista) só
    // tinha o teto geral de nível (0-5) e nenhuma ligação com pool
    // nenhum — dava pra deixar qualquer perícia em 5 de graça, quantas
    // vezes quisesse, sempre que a Criação ou o Level Up estivessem
    // ativos. Por isso, pra quem não é Mestre, o valor digitado no campo
    // Nível é ignorado: uma perícia nova sempre nasce em 0 (só serve pra
    // cadastrar/descrever), e uma perícia existente mantém o nível que já
    // tinha — mudar o nível de verdade exige passar pelos fluxos acima.
    const existente = id ? fichaAtual.pericias[id] : null;
    const nivel = isMestre
        ? Math.max(0, Math.min(5, Number(el.modalNivel.value) || 0))
        : (existente ? existente.nivel : 0);
...
function montarDistribuidorBonus(c, onMudou, container) {
    ...
    ATRIBUTOS_PRIMARIOS.forEach(attr => {
        ...
        //
        // BUGFIX: as duas escritas (dados e depois criacao via
        // salvarEstadoCriacao) eram sequenciais e SEM pausarSync. A
        // primeira já disparava o listener onValue, que substituía
        // fichaAtual inteiro (inclusive .criacao) por um snapshot do
        // banco ainda SEM o desconto que a gente acabou de fazer em
        // memória. Quando a segunda escrita rodava, ela lia esse
        // fichaAtual.criacao já substituído (sem o decremento) e gravava
        // ele de volta por cima — cancelando o gasto. Resultado: o
        // atributo subia de verdade, mas o pool de pontos bônus nunca
        // baixava, então dava pra clicar "+" sem limite. Corrigido
        // gravando dados+criacao num ÚNICO update() atômico (mesmo
        // padrão de salvarWizardStep), com pausarSync/retornarSync como
        // proteção extra contra corrida em cliques muito rápidos.
        linha.querySelector(".btn-menos").addEventListener("click", async () => {
            ...
            pausarSync();
            try {
                await update(ref(db, `fichas/${fichaAtualId}`), { dados: fichaAtual.dados, criacao: fichaAtual.criacao });
            } finally {
                retornarSync();
            }
            onMudou();
    ...
    // Mesmo bug e mesma correção do bloco de atributos acima: um único
    // update() atômico cobrindo pericias+dados+criacao, protegido por
    // pausarSync/retornarSync.
...
function renderPassoLevelUp() {
    ...
    if (lvl.passo === 1) {
        ...
        if (!lvl.dadoVidaRolado) {
            ...
            btn.addEventListener("click", async () => {
                executarPassoDadoVida(fichaAtual);
                await salvarEstadoLevelUp();
                renderPassoLevelUp();
            });
