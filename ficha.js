...
function renderPassoLevelUp() {
    ...
    if (lvl.passo === 1) {
        ...
        if (!lvl.dadoVidaRolado) {
            ...
            btn.addEventListener("click", async () => {
                ...
                if (r) {
                    ...
                    await registrarRolagem({
                        ...
                        modificador: r.bonus,
                        resultado: r.total,
                        detalhe: `Rolagem de PV: ${r.rolagem}. Valor mínimo ajustado (se necessário): ${r.valorFinal}. Bônus de CON: ${r.bonus}. Total aplicado ao HP: ${r.total}`
                    });
                }
                renderPassoLevelUp();
            });
            ...
            const detalheAjuste = r.ajustado ? ` (ajustado pro mínimo de ${r.minimo})` : "";
            el.levelupCorpo.innerHTML += `<p class="entity-nome">1d${r.faces} (${r.rolagem}${detalheAjuste}) + ${r.bonus} = +${r.total} PV</p>`;
            const btn = document.createElement("button");
