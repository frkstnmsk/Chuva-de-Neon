...
async function carregarCarregador(carregadorId, carregadorItem) {
    ...
    for (const proj of candidatos) {
        ...
        if (restante > 0) {
            ...
            delete inventarioAtualizado[proj.id];
        }
    ...
    fichaAtual.inventario = inventarioAtualizado;
...
function atualizarCamposPorTag(tagKey, nivelTag, armaConfig, periciaUsoAtual, classeProtecaoAtual, reducoesDanoAtuais, carregadorConfigAtual, projetilConfigAtual) {
    ...
    // Projétil — quantidade de projéteis que este item representa.
    const exigeQuantidade = tagExigeQuantidadeProjetil(tagKey);
    el.modalCampoProjetilQuantidade.style.display = exigeQuantidade ? "flex" : "none";
    if (exigeQuantidade) el.modalProjetilQuantidade.value = (projetilConfigAtual && projetilConfigAtual.quantidade) ?? 0;
...
async function salvarItemDoModal(id) {
    ...
    // Projétil — quantidade de rounds que esse item representa.
    let projetil = null;
    if (tagExigeQuantidadeProjetil(tag)) {
        projetil = { quantidade: Number(el.modalProjetilQuantidade.value) || 0 };
    }
