...
function montarGridsEstaticas() {
    ...
    ATRIBUTOS_PRIMARIOS.forEach(attr => {
        ...
        card.querySelector(`[data-rolar-attr="${attr.key}"]`).addEventListener("click", async () => {
            ...
            // "atributo:X" ou "rolagem de X" etc.) nunca entrava aqui, só
            // os Ocasionais que o próprio jogador liga na hora (modal logo
            // abaixo, ver rolarComPossibilidadeDeOcasionais). Agora soma os
            // dois — vale pra Constituição também (a exceção dela é só
            // PV/Energia e o teste de Sangramento, ver atributoPrimarioEfetivo
            // em regras.js). atributoParaRolagemPropria já inclui tanto
            // "atributo:X" quanto "rolagem:X" (ver regras.js) — só essa
            // rolagem soma os dois; o resto do sistema (escala, carga,
            // secundários, defesa) usa só atributoPrimarioEfetivo.
            const totalAttr = atributoParaRolagemPropria(estado.fichaAtual.dados, attr.key, modificadoresAtuais());
