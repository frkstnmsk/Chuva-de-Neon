...
import {
    ...
    rotuloCampoClasseProtecao,
    CALIBRES, rotuloCalibre, tagExigeCalibre, rotuloCampoCalibre, calibresCompativeis,
...
const el = {
    ...
    modalLabelCalibre: document.getElementById("modal-label-calibre"),
...
function renderizarAtributos(modificadoresPlanos) {
    ...
    RECURSOS.forEach(rec => {
        ...
        const total = Math.round(derivados.recursos[rec.key].total) + bonusExtra;
...
document.addEventListener("input", (e) => {
    ...
    if (valor !== null && !Number.isNaN(valor)) {
        ...
        const max = Math.round(derivados.recursos[recursoKey].total) + bonusExtra;
...
function prepararModalItem(existente, ehBanco) {
    ...
    if (existente) {
        ...
        atualizarCamposPorTag(existente.tag, existente.nivelTag, existente.arma, existente.periciaUso, existente.classeProtecao, existente.reducoesDano, existente.carregador, existente.projetil, existente.calibre);
    } else {
        el.modalNome.value = "";
        el.modalTag.value = "";
    ...
    const renderSugestoes = () => {
        ...
        encontrados.forEach(it => {
            ...
            div.addEventListener("click", () => {
                ...
                atualizarCamposPorTag(it.tag, it.nivelTag, it.arma, it.periciaUso, it.classeProtecao, it.reducoesDano, it.carregador, it.projetil, it.calibre);
                el.modalItemBancoOpcoes.style.display = "none";
...
// colete). Não tem mais nada a ver com casar arma x carregador x
// projétil, isso agora é papel do campo Calibre, à parte.
function atualizarVisibilidadeClasseProtecao(classeAtual) {
    ...
    if (exige) {
        if (el.modalLabelClasseProtecao) el.modalLabelClasseProtecao.innerText = rotuloCampoClasseProtecao();
        popularClassesProtecao(classeAtual);
    }
}

function popularCalibres(calibreAtual) {
    ...
    el.modalCalibre.value = (calibreAtual && CALIBRES.some(c => c.key === calibreAtual)) ? calibreAtual : CALIBRES[0].key;
}

// Reavalia se o campo "Calibre" deve aparecer: arma de fogo, carregador
// e projétil exigem — é o que casa os três (uma arma de calibre X só
// carrega carregador/projétil de calibre X; exceção: escopeta 12 Gauge
// aceita Buckshot ou Slug, ver calibresCompativeis).
function atualizarVisibilidadeCalibre(calibreAtual) {
    const tagKey = el.modalTag.value;
    const periciaAtual = el.modalCampoPericiaUso.style.display !== "none" ? el.modalPericiaUso.value : null;
    const exige = tagExigeCalibre(tagKey, periciaAtual);
    el.modalCampoCalibre.style.display = exige ? "flex" : "none";
    if (exige) {
        if (el.modalLabelCalibre) el.modalLabelCalibre.innerText = rotuloCampoCalibre();
        popularCalibres(calibreAtual);
    }
}
...
// que casam com o Calibre selecionado na arma (campo próprio, separado
// da Classe de Proteção). Se o calibre ainda não tiver sido escolhido,
// mostra todos os carregadores do inventário.
...
function atualizarCamposPorTag(tagKey, nivelTag, armaConfig, periciaUsoAtual, classeProtecaoAtual, reducoesDanoAtuais, carregadorConfigAtual, projetilConfigAtual, calibreAtual) {
    ...
    // Classe de Proteção — colete e arma de fogo exigem (dano x colete).
    atualizarVisibilidadeClasseProtecao(classeProtecaoAtual);
    // Calibre — arma de fogo, carregador e projétil exigem (casa os três).
...
async function salvarItemDoModal(id) {
    ...
    const exigeCalibre = tagExigeCalibre(tag, periciaUso);
