        ...
        itensCategoria.forEach(([id, it]) => {
            ...
            li.innerHTML = `
                <div class="entity-main">
                    ...
                    <span class="entity-sub">${tagLabel} · ${it.peso || 0} kg${periciaLabel}${classeLabel}</span>
                </div>
                <div class="entity-badges">
    ...
    if (!armas.length) {
        ...
        armas.forEach(arma => {
            ...
            li.querySelector(".btn-usar-item").addEventListener("click", async (e) => {
                e.stopPropagation();
                if (!podeUsar) return;
                await rolarUsoItem(arma, modificadoresPlanos);
            });
...
function prepararModalItem(existente) {
    ...
    if (existente) {
        ...
        atualizarCamposPorTag(existente.tag, existente.nivelTag, existente.arma, existente.periciaUso, existente.classeProtecao);
    } else {
        el.modalNome.value = "";
        el.modalTag.value = "";
        el.modalPeso.value = 0;
        el.modalCategoriaItem.value = categoriaInventarioAtiva || "levando";
        atualizarCamposPorTag("", null, null, null, null);
    }
}
...
function atualizarCamposPorTag(tagKey, nivelTag, armaConfig, periciaUsoAtual, classeProtecaoAtual) {
...
document.getElementById("modal-tag")?.addEventListener("change", (e) => {
    atualizarCamposPorTag(e.target.value, null, null, null, null);
});
