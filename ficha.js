...
async function rolarERegistrar(nomeAlvo, modificador) {
    ...
    // sobrando nesse turno. Consome 1 ação por rolagem feita.
    if (!isMestre && combateComIniciativaAtivo()) {
        ...
        if (meuId) {
            ...
            consumirAcaoCombate(meuId); // não bloqueia a rolagem esperando a escrita
        }
    }
    ...
    const linhas = ordemTurnos.map(pid => {
        ...
        return `
            <div class="combate-linha ${ativo ? "combate-linha-ativa" : ""}">
                <span class="combate-nome">${escapeHtml(p.nome)}${marcadorVoce}</span>
...
function montarGerenciadorCombate(corpo) {
    ...
    function renderIniciativa() {
        ...
        ordemTurnos.forEach(pid => {
            ...
            linha.innerHTML = `
                <span class="combate-nome">${escapeHtml(p.nome)}</span>
