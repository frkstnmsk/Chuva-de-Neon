...
async function resolverAtaque(it, modificadoresPlanosAtacante, participante) {
    ...
    if (combateComIniciativaAtivo() && participante.esquivaDisponivel) {
        await abrirReacaoPendente({
            ...
            danoTotal, tipoDanoKey, tipoDanoLabel,
...
function renderizarReacaoPendente(r) {
    el.reacaoDefesaCorpo.innerHTML = `
        <p class="hint">${escapeHtml(r.nomeAtacante)} acertou ${escapeHtml(r.nomeAlvo)} com ${escapeHtml(r.nomeArma)} (${r.resultadoAtaque} vs. dificuldade ${r.dificuldade}). Dano previsto: ${r.danoTotal} (${escapeHtml(r.tipoDanoLabel)}).</p>
