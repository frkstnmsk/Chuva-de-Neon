...
function renderizarInventario(modificadoresPlanos) {
    ...
    if (!itensCategoria.length) {
        ...
        itensCategoria.forEach(([id, it]) => {
            ...
            const classeLabel = it.classeProtecao ? ` · Classe de Proteção/Calibre ${escapeHtml(rotuloClasseProtecao(it.classeProtecao))}` : "";
...
function renderizarCombate() {
    ...
    if (!armas.length) {
        ...
        armas.forEach(arma => {
            ...
            const classeLabel = arma.classeProtecao ? ` · Classe de Proteção/Calibre ${escapeHtml(rotuloClasseProtecao(arma.classeProtecao))}` : "";
...
function atualizarVisibilidadeClasseProtecao(classeAtual) {
    ...
    if (exige) {
        // Mesmo campo/select serve pra dois papéis: em colete é a Classe de
        // Proteção (até que calibre ele aguenta); em carregador/projétil/
        // arma de fogo é o Calibre (o que casa os três entre si) — o texto
        // do rótulo muda conforme a tag (rotuloCampoCalibre já cobre isso).
...
function popularCarregadorAnexado(carregadorIdAtual) {
    ...
    // O mesmo select de Classe de Proteção é quem carrega o Calibre da
    // arma quando a tag é "arma" (ver rotuloCampoCalibre/tagExigeClasseProtecao).
...
function atualizarCamposPorTag(tagKey, nivelTag, armaConfig, periciaUsoAtual, classeProtecaoAtual, reducoesDanoAtuais, carregadorConfigAtual, projetilConfigAtual) {
    ...
    // Classe de Proteção / Calibre — mesmo campo cobre os dois papéis
    // (colete usa como classe de proteção; carregador/projétil/arma de
    // fogo usam como calibre, pra casar os três entre si).
...
async function salvarItemDoModal(id) {
    ...
    if (exigeClasseProtecao && !classeProtecao) { toast("Escolha a classe de proteção/calibre deste item.", "erro"); return; }
