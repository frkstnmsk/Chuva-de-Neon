...
async function init() {
    ...
    await garantirCalendarioInicial();
    configurarCalendario();
    configurarLogDados();
    configurarAvisoCustoVida();
    configurarPopupTreinamento();
    configurarGodmode();

    configurarBotoesAdicionar();
    configurarModal();
    configurarBuscaPericia();
    configurarModificacoesArma();
    configurarModificadoresGenerico();
}
...
function montarSelectsFixos() {
    ...
    // ---- Manobras de combate (lista fixa, monta 1x) ----
    el.listaManobrasCombate.innerHTML = "";
    MANOBRAS_COMBATE.forEach(m => {
        const li = document.createElement("li");
        li.innerHTML = `
            <div class="entity-main">
                <span class="entity-nome">${m.nome}</span>
                <span class="entity-sub">${m.pericias.join(", ")} · dif.: ${m.dificuldade}</span>
                <span class="entity-sub">${m.efeito}</span>
            </div>
            <span class="manobra-alcance">${m.alcance}</span>
        `;
...
function podeEditarPericiaAtributo() {
    // "Regra de ouro": fora de criação/levelup/treino, só o Mestre com
    // godmode ativo pode tocar em atributo/perícia.
...
function configurarLogDados() {
    ouvirLogDados((lista) => {
        ...
        if (!lista.length) {
            el.logDadosLista.innerHTML = `<li>Nenhuma rolagem ainda.</li>`;
            return;
        }
        lista.forEach(entrada => {
            const li = document.createElement("li");
            const modText = entrada.modificador ? ` (${entrada.modificador >= 0 ? "+" : ""}${entrada.modificador})` : "";
            li.innerHTML = `<span class="log-quem">${escapeHtml(entrada.quem)}</span>${modText}: <span class="log-resultado">${entrada.resultado}</span>${entrada.detalhe ? ` — ${escapeHtml(entrada.detalhe)}` : ""}`;
    ...
    el.logRolarBtn.addEventListener("click", async () => {
        ...
        await registrarRolagem({ quem, modificador, resultado, detalhe: `d20 (${bruto}) ${modificador >= 0 ? "+" : ""}${modificador}` });
        toast(`Rolagem livre: ${resultado}.`);
    });
}
...
function abrirAcaoMestre(acao) {
    ...
    if (acao === "xp") {
        const select = criarSelectFichas();
        const input = document.createElement("input");
        input.type = "number"; input.placeholder = "Quantidade de XP"; input.value = 50;
        const btn = document.createElement("button");
        btn.className = "btn-lime"; btn.type = "button"; btn.innerText = "Enviar XP";
        btn.addEventListener("click", async () => {
            if (!select.value) { toast("Escolha uma ficha.", "erro"); return; }
            await darXp(select.value, Number(input.value) || 0);
            toast(`XP enviado para ${nomeDeFicha(select.value)}.`);
        });
