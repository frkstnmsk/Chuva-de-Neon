...
const CAMPOS_SO_MESTRE = ["nivel", "xp", "dinheiroLimpo", "dinheiroSujo"];
...
function renderizarFinancas() {
    el.financasSaldoHint.innerText = isMestre
        ...
        : "apenas o Mestre pode adicionar dinheiro — use \"Gastar dinheiro\" abaixo pra remover";
...
function configurarFinancas() {
    ...
    el.financasGastarBtn.addEventListener("click", async () => {
        ...
        const origem = el.financasGastarOrigem.value;
        const saldoAtual = origem === "sujo" ? (Number(fichaAtual.dados.dinheiroSujo) || 0) : (Number(fichaAtual.dados.dinheiroLimpo) || 0);
        ...
        await criarAcaoPendente({
            ...
            nomeJogador,
            detalhe: `${nomeJogador} quer gastar CN$ ${valor} (${origem === "sujo" ? "dinheiro sujo" : "dinheiro limpo"}).`,
            payload: { valor, origem }
        });
...
// atacante, compara com a dificuldade defensiva do alvo (10 + Agilidade
// ou Constituição, dependendo da perícia da arma), e se acertar, resolve
// o dano (base da arma + escala) descontando a redução de armadura do
// alvo — tudo registrado numa única linha no Log de Dados.
async function resolverAtaque(it, modificadoresPlanosAtacante, participante) {
    ...
    const modAtaque = modificadorDePericiaComPenalidade(nomePericia, fichaAtual.dados, fichaAtual.pericias, modificadoresPlanosAtacante);
    ...
    if (!acertou) {
        const detalhe = `${nomeAtacante} atacou ${nomeAlvo} com ${it.nome} (${nomePericia}). Ataque Falhou — d20 ${brutoAtaque} ${sinalMod}${modAtaque} = ${resultadoAtaque} vs. dificuldade ${dificuldade}.`;
        await registrarRolagem({ quem: nomeAtacante, modificador: modAtaque, resultado: resultadoAtaque, detalhe });
        toast(detalhe, "erro");
        return;
    }

    // Acerto — resolve dano: base da arma + bônus de escala (se corpo a
    // corpo), usando o atributo-base da própria perícia da arma.
    ...
    const danoTotal = (Number(armaConfig.danoBase) || 0) + bonusEscala;
    const tipoDanoKey = armaConfig.tipoDano;
    ...
    const detalheDano = resultadoDano.reducao > 0
        ? `${nomeAtacante} atacou ${nomeAlvo} com ${it.nome}. Acerto! (${resultadoAtaque} vs. dificuldade ${dificuldade}) Dano: ${resultadoDano.danoBruto} (${tipoDanoLabel}) - ${resultadoDano.reducao} (redução) = ${resultadoDano.danoFinal} de dano aplicado. PV restante: ${resultadoDano.novoPv}.`
        : `${nomeAtacante} atacou ${nomeAlvo} com ${it.nome}. Acerto! (${resultadoAtaque} vs. dificuldade ${dificuldade}) Dano: ${resultadoDano.danoFinal} (${tipoDanoLabel}) aplicado. PV restante: ${resultadoDano.novoPv}.`;
    ...
    if (!itensCategoria.length) {
        ...
        itensCategoria.forEach(([id, it]) => {
            ...
            li.querySelector(".btn-usar-item").addEventListener("click", async (e) => {
                e.stopPropagation();
                if (!podeUsar) return;
                await iniciarUsoItem(it, modificadoresPlanos);
            });
        ...
        armas.forEach(arma => {
            ...
            li.innerHTML = `
                <div class="entity-main">
                    ...
                    <span class="entity-sub">Dano: ${escapeHtml(cfg.dano || "—")}${tipoDano ? " · " + tipoDano.label : ""}${escala ? " · " + escala.label : ""}${periciaLabel}${classeLabel}</span>
...
function prepararModalParaLista(lista, objetoExistente) {
    ...
    aplicarSomenteLeituraModal(somenteLeituraItem);
    if (somenteLeituraItem) {
...
// ou a perícia da arma mudam — porque uma arma pode alternar entre
// "arma de fogo" (exige classe) e arma branca (não exige) sem fechar
// o modal, só trocando a perícia de combate no dropdown.
...
function atualizarCamposPorTag(tagKey, nivelTag, armaConfig, periciaUsoAtual, classeProtecaoAtual, reducoesDanoAtuais) {
    ...
    // Classe de Proteção — obrigatória em coletes (sempre) e em armas de
    // fogo (depende de qual perícia de combate foi selecionada acima).
...
// Proteção sem precisar trocar a tag — reavalia na hora.
...
function montarPainelNpcs(corpo) {
    ...
    montarFormularioNpcRapido(corpo, async () => { toast("NPC criado."); });
}
