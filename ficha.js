...
const el = {
    ...
    // btnAddPericia removido: perícia só entra por Criação, Treinamento ou Level Up (ver renderizarPericias)
...
async function rolarERegistrar(nomeAlvo, modificador, infoExtra = "") {
    ...
    const sufixo = infoExtra ? ` | ${infoExtra}` : "";
    const quem = isMestre ? `Mestre (${nomeDeFicha(fichaAtualId) || "—"})` : (fichaAtual?.config?.nomeExibicao || sessao.nome || "Jogador");
    await registrarRolagem({ quem, modificador, resultado, detalhe: `${nomeAlvo}: d20 (${bruto}) ${modificador >= 0 ? "+" : ""}${modificador}${sufixo}` });
    toast(`${nomeAlvo}: ${resultado} (d20: ${bruto} ${modificador >= 0 ? "+" : ""}${modificador})${sufixo}`);
...
async function resolverAtaque(it, modificadoresPlanosAtacante, participante) {
    ...
    // Crítico (manual pg. 12): resultado igual ou maior que 20 na rolagem
    // de combate — sem distinção entre 20 natural ou 20 com bônus somado —
    // dobra o dano do golpe. Aplicado sobre o dano bruto, antes de
    // Esquiva/Bloqueio (que age em cima do dano já dobrado, igual a
    // qualquer outro modificador de redução).
    const foiCritico = resultadoAtaque >= 20;
    if (foiCritico) {
        danoTotal *= 2;
    }
    ...
    const detalheDano = resultadoDano.reducao > 0
        ? `${nomeAtacante} atacou ${nomeAlvo} com ${it.nome}. ACERTO!${criticoTexto} (${resultadoAtaque} vs. dificuldade ${dificuldade}${recuoTexto}${precisaoTexto}) Dano: ${resultadoDano.danoBruto} (${tipoDanoLabel}) - ${resultadoDano.reducao} (redução) = ${resultadoDano.danoFinal} de dano aplicado. PV restante: ${resultadoDano.novoPv}.${efeitoTexto}`
        : `${nomeAtacante} atacou ${nomeAlvo} com ${it.nome}. ACERTO!${criticoTexto} (${resultadoAtaque} vs. dificuldade ${dificuldade}${recuoTexto}${precisaoTexto}) Dano: ${resultadoDano.danoFinal} (${tipoDanoLabel}) aplicado. PV restante: ${resultadoDano.novoPv}.${efeitoTexto}`;
            ...
            btn.addEventListener("click", async (e) => {
                ...
                if (ehGolpeDesarmadoComDano(m.nome)) {
                    ...
                    if (combateTemParticipantes()) {
                        ...
                        await rolarERegistrar(
                            `${m.nome} (${nomePericia}) — Acerto`,
                            calc.total,
                            `Dano potencial: ${danoCalc.total} (${dadoTexto} + ${danoCalc.bonusEscala})`
                        );
                    }
                    return;
                }
