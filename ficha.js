...
import {
    ...
    calcularDanoDesarmado, calcularDificuldadeArmaFogo
} from "./regras.js";
import {
...
function configurarModalSelecionarAlvo() {
    ...
    el.alvoConfirmar.addEventListener("click", async () => {
        ...
        el.modalSelecionarAlvo.classList.remove("active");
        contextoAtaque = null;
        await resolverAtaque(item, modificadoresPlanos, participante);
    });
}
...
async function resolverAtaque(it, modificadoresPlanosAtacante, participante) {
    ...
    // Acerto — resolve dano. Golpe desarmado usa 1dForça + Força [escala]
    // (manual pg. 49-50); arma cadastrada usa dano base + bônus de
    // escala corpo a corpo (armas de fogo não têm escala, só dano base).
            ...
            btn.addEventListener("click", async (e) => {
                ...
                if (ehGolpeDesarmadoComDano(m.nome)) {
                    const itemDesarmado = {
                        ...
                        arma: { danoBase: 0, escala: null, escalaMult: ESCALA_MULT_DESARMADO[m.nome], tipoDano: "contusao", desarmado: true }
                    };
                    if (combateTemParticipantes()) {
                        ...
                        const danoCalc = calcularDanoDesarmado(forcaAtual, ESCALA_MULT_DESARMADO[m.nome]);
                        await rolarERegistrar(`${m.nome} (${nomePericia}) · dano potencial ${danoCalc.total} (1d${danoCalc.faces}: ${danoCalc.dado} + ${danoCalc.bonusEscala})`, calc.total);
                    }
                    return;
                }
...
// =====================================================================
function montarGerenciadorCombate(corpo) {
    ...
    // ---- Avançar turno (zera Recuo acumulado) ----
    const secaoTurno = document.createElement("div");
    secaoTurno.className = "section-header";
    secaoTurno.innerText = "Turno";
    corpo.appendChild(secaoTurno);
    const avisoTurno = document.createElement("p");
    avisoTurno.className = "hint";
    avisoTurno.innerText = "O modificador de Recuo das armas de fogo acumula por disparo dentro do turno de cada personagem. Clique aqui ao passar a vez pro próximo personagem pra zerar a contagem de todo mundo.";
    corpo.appendChild(avisoTurno);
    const btnNovoTurno = document.createElement("button");
    btnNovoTurno.className = "btn-blue"; btnNovoTurno.type = "button"; btnNovoTurno.innerText = "Novo turno (zerar Recuo)";
    btnNovoTurno.addEventListener("click", async () => { await resetarDisparosTurno(); });
    corpo.appendChild(btnNovoTurno);
