...
export function rolarDadoVida(constituicao) {
    ...
    // Regra de mínimo: o resultado bruto não pode ser menor que
    // metade do valor do dado + 1. Se for, o valor do dado usado no
    // cálculo é substituído por esse mínimo antes de somar o bônus de
    // CON. `rolagem` guarda o resultado bruto (pro log mostrar o que
    // realmente saiu no dado); `valorFinal` é o que efetivamente conta.
    const minimo = Math.floor(faces / 2) + 1;
    const ajustado = rolagem < minimo;
    const valorFinal = ajustado ? minimo : rolagem;
    return { faces, bonus, rolagem, minimo, ajustado, valorFinal, total: valorFinal + bonus };
}
