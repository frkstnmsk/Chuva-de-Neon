...
export function tagExigeClasseProtecao(tagKey, periciaUso) {
    // Armas de fogo pararam de usar Classe de Proteção pra combate: agora
    // usam Dificuldade de Acerto própria (pg. 95-97). Classe de Proteção
    // segue existindo só do lado do colete/placa.
...
export const PADROES_RECUO = [
    ...
    { key: "bipe", label: "Só controlável com bipé/apoio (–2 em todos os disparos sem apoio)" }
];
...
export function modificadorRecuo(padraoKey, numeroDoTiro) {
    ...
    if (n <= 1) return padraoKey === "bipe" ? -2 : 0;
    switch (padraoKey) {
...
// Rótulo especial pra vincular uma arma sem nenhuma perícia de combate —
// fica sempre por último na lista, igual uma opção "nenhuma"/"outros".
// A rolagem com essa opção cai automaticamente na regra geral de "sem
// perícia" do manual (pg. 21: "Tentar uma tarefa sem a perícia recebe a
// penalidade de -1."), porque modificadorDePericiaComPenalidade() já
// devolve -1 sempre que o nome não bate com nenhuma perícia real da
// ficha — não precisa de nenhum tratamento especial em outro lugar.
export const SEM_PERICIA = "Sem Perícia";
export const PERICIAS_ARMA_COMBATE_COM_SEM_PERICIA = [...PERICIAS_ARMA_COMBATE, SEM_PERICIA];
...
export function periciasVinculaveisPorTag(tagKey) {
    switch (tagKey) {
        case "arma": return PERICIAS_ARMA_COMBATE_COM_SEM_PERICIA;
