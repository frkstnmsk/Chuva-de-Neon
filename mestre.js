...
export async function responderReacaoPendente(escolha) {
    ...
    const detalheDano = resultadoDano.reducao > 0
        ? `${r.nomeAtacante} atacou ${r.nomeAlvo} com ${r.nomeArma}. ACERTO! (${r.resultadoAtaque} vs. dificuldade ${r.dificuldade}${r.recuoTexto || ""}${r.precisaoTexto || ""}) ${notaEscolha} Dano: ${resultadoDano.danoBruto} (${r.tipoDanoLabel}) - ${resultadoDano.reducao} (redução) = ${resultadoDano.danoFinal} de dano aplicado. PV restante: ${resultadoDano.novoPv}.${efeitoTexto}`
        : `${r.nomeAtacante} atacou ${r.nomeAlvo} com ${r.nomeArma}. ACERTO! (${r.resultadoAtaque} vs. dificuldade ${r.dificuldade}${r.recuoTexto || ""}${r.precisaoTexto || ""}) ${notaEscolha} Dano: ${resultadoDano.danoFinal} (${r.tipoDanoLabel}) aplicado. PV restante: ${resultadoDano.novoPv}.${efeitoTexto}`;
