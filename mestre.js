...
export async function passarODia(calendarioAtual, fichasAtivas) {
    ...
    if (virouDomingo) {
        ...
        // exigindo confirmação via aviso). Vai sempre pro Dinheiro Limpo.
        for (const [fichaId, ficha] of Object.entries(fichasAtivas)) {
            const ganhoFixo = Number(ficha.dados && ficha.dados.ganhoFixo) || 0;
            if (ganhoFixo > 0) {
                const atual = Number(ficha.dados.dinheiroLimpo) || 0;
                await update(ref(db, `fichas/${fichaId}/dados`), { dinheiroLimpo: atual + ganhoFixo });
            }
        }
    }
...
export async function confirmarAcaoPendente(acao) {
    ...
    if (tipo === "remover_item") {
        ...
        const campo = payload.origem === "sujo" ? "dinheiroSujo" : "dinheiroLimpo";
        const snap = await get(ref(db, `fichas/${fichaId}/dados/${campo}`));
        const atual = snap.exists() && snap.val() !== null ? Number(snap.val()) : 0;
        await update(ref(db, `fichas/${fichaId}/dados`), { [campo]: atual - Number(payload.valor || 0) });
